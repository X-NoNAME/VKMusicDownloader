    ...
    private class MyRunnable implements Callable<List<AudioFile>> {
        ...
        public List<AudioFile> call() throws Exception {
            List<AudioFile> urls = new ArrayList<>();
                ...
                if(frame.from.getText().indexOf("http://")==0){
                    frame.jTextArea1.append("Start serching on page "+frame.from.getText()+"\n\n");
                    driver.get(frame.from.getText());
                    ...
                    driver.findElement(By.xpath("//button[contains(@onclick,\"quick_login\")]")).click();
                    ...
                    frame.jTextArea1.append("Start serching on yours audio page;\n\n");
                    driver.findElement(By.linkText("Мои Аудиозаписи")).click();
                }
                Thread.sleep(1000);
                System.out.println( ">>>"+driver.findElements(By.xpath("//input[@type='hidden']")));
                
                List<WebElement> divs = driver.findElements(By.xpath("//input[@type='hidden']"));
                
                int max = Integer.parseInt(frame.max.getText().trim());
